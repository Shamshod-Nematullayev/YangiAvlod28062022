                ...
                .then(result=>{
                    if(result){
                        ...
                        .then(()=>{
                            UserVerification.deleteOne({userId})
                            .then(()=>{
                                res.render("verified.ejs", {DOMAIN: process.env.DOMAIN})
                            })
                            .catch(err=>{
                                console.log(err);
...
router.post('/loginAdmin', async (req, res)=>{
    try{
       const user = await User.findOne({email: req.body.email});
       if(!user)  {res.status(404).render('adminLogin.ejs',{message:"user not found"} )}
       ...
       if(user.verified){
           if(user.isAdmin){
            ...
            res.status(200).render('dashboard.ejs', {adminID: user._id, students})
