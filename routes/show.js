...
route.get('/admin',async (req, res)=>{
    ...
    if(user.isAdmin){ 
        ...
        return res.status(200).render('dashboard.ejs', {adminID: user._id, students})
    }{
...
route.get('/addStudent/:adminID',async (req, res) =>{
    try {
        ...
        if(!user){
            ...
            if(user.isAdmin){
                res.status(200).render('addUser/index.ejs', {adminID: req.params.adminID });
            }else{
                res.status(400).send('Siz bu sahifaga kirish huquqiga ega emassiz')
            }
        }
